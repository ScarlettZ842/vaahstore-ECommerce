...
let ajax_url = base_url + "/api/store/payments";
...
export const usePaymentStore = defineStore({
  ...
  getters: {
    filteredOrders() {
      ...
      const route_search_query =
        this.route.query.filter && this.route.query.filter.order
          ? decodeURIComponent(this.route.query.filter.order)
              .toLowerCase()
              .trim()
          : "";
      ...
      if (key || route_search_query) {
        return this.item.order_payments.filter((order_payment) => {
          ...
          return (
            (!key || order_user_name.includes(key)) &&
            (!route_search_query ||
              order_user_name.includes(route_search_query))
          );
        });
      } else {
  ...
  actions: {
    ...
    setViewAndWidth(route_name) {
      ...
      if (
        route_name.includes("payments.view") ||
    ...
    searchPaymentMethod(event) {
      this.payment_method_suggestion = this.payment_methods.filter(
        (payment_methods) => {
    ...
    addOrderFIlter() {
      const unique_order = Array.from(
        new Set(this.selected_order.map((v) => v.id))
      );
      this.selected_order = unique_order.map((id) =>
        this.selected_order.find((v) => v.id === id)
      );
    ...
    async getOrdersByName() {
      let decodedOrders = this.query.filter.order.map((order) =>
