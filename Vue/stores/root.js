...
let base_element = document.getElementsByTagName("base")[0];
let base_url = base_element
  ...
  actions: {
    async getAssets() {
      if (this.assets_is_fetching === true) {
        ...
        vaah().ajax(this.ajax_url + "/assets", this.afterGetAssets);
      }
    },
    ...
    async afterGetAssets(data, res) {
      if (data) {
        ...
        this.selected_store_at_sidebar =
          data.selected_store_at_sidebar ?? data.default_store;
        ...
        this.assets = {
          ...
          default_store: null,
          selected_store_at_sidebar: null,
          charts_data_filtered_by: '{"today":true}',
        };
        this.stores = [];
      }
    },
    async updatedChartsDateFilter(chart_date) {
      ...
      if (
        charts_filter_by_date?.custom?.start_date &&
        charts_filter_by_date?.custom?.end_date
      ) {
    ...
    initWatchStoreChange(route, store, callback) {
      // Accepting 'route' and 'store' as parameters
      if (!route) {
        return;
      }

      watch(
        () => route.query.selected_store,
        async (newVal, oldVal) => {
