...
let ajax_url = base_url + "/api/store/products";
...
export const useProductStore = defineStore({
  ...
  getters: {
    ...
    getLeftColumnClasses: (state) => {
      ...
      if (state.isMobile && state.view !== "list") {
        return null;
      }
    ...
    getRightColumnClasses: (state) => {
      ...
      if (
        state.isMobile &&
        (state.view === "list-and-item" || state.view === "list-and-filters")
      ) {
        return "w-full";
      }
  ...
  actions: {
    ...
    setViewAndWidth(route_name) {
      ...
      if (
        route_name.includes("products.view") ||
    ...
    watchItem() {
      if (this.item) {
        ...
        watch(
          () => this.variation_item.attribute_option_type,
          (newVal, oldVal) => {
            if (newVal != oldVal) {
    ...
    watchQuantity() {
      watch(
        () => [this.quantity?.from, this.quantity?.to],
        ([min, max]) => {
    ...
    async bulkRemoveAttribute() {
      let selected_attribute = this.variation_item.product_attributes.filter(
      ...
      if (selected_attribute.length === 0) {
        ...
        temp.length === this.variation_item.product_attributes.length
      ) {
        this.variation_item.product_attributes = [];
      } else {
    ...
    async bulkRemoveVendor() {
      let selected_vendors = this.item.vendors.filter(
        (vendor) => vendor.is_selected
      );
      let temp = null;
      this.select_all_vendor = false;
    ...
    async getAttributeList(callback = null, get_attribute_from_group = false) {
      let params = {
        attribute_type:
          this.variation_item.attribute_option_type == 0
            ? "attribute"
            : "attribute_group",
    ...
    addNewProductAttribute(data = null) {
      ...
      if (
        ...
        if (
          this.variation_item.product_attributes.some(
            (attr) => attr.id === new_attribute.id
          )
        ) {
        ...
        this.variation_item.product_attributes = this.getUnique(
          this.variation_item.product_attributes,
          (it) => it.id
        );
      } else if (
    ...
    removeProductAttribute(attribute) {
      this.variation_item.product_attributes =
        this.variation_item.product_attributes.filter(function (item) {
    ...
    generateProductVariation() {
      if (
        this.variation_item.product_attributes &&
        this.variation_item.product_attributes.length > 0
      ) {
    ...
    removeProductVariation(item) {
      let item_key = this.getIndexOfArray(
        this.item.product_variations.structured_variations,
        item
      );
      if (item_key >= 0) {
    ...
    bulkRemoveProductVariation() {
      let selected_variation =
        this.item.product_variations.structured_variations.filter(
          (variation) => variation.is_selected
        );
      let temp = null;
      this.variation_item.select_all_variation = false;
      temp = this.item.product_variations.structured_variations.filter(
        (item) => {
          return item["is_selected"] === false;
        }
      );
    ...
    //---------------------------------------------------------------------
    setDefault() {
      this.item.product_variations.structured_variations.forEach(
        (variation) => {
    ...
    addNewProductVariation(new_record) {
      if (
        ...
        Object.keys(this.variation_item.new_variation).length >
          Object.keys(
            this.variation_item.create_variation_data.all_attribute_name
          ).length
      ) {
        if (
          this.item.product_variations &&
          Object.keys(this.item.product_variations).length > 0
        ) {
          let error_message = [];
          ...
          this.item.product_variations.structured_variations.forEach((i, k) => {
            if (
              i.variation_name ==
              this.variation_item.new_variation.variation_name.trim()
            ) {
              error_message.push("variation name must be unique");
            }
            this.variation_item.create_variation_data.all_attribute_name.forEach(
              (i_new, k_new) => {
                if (
                  i[i_new]["value"] ==
                  this.variation_item.new_variation[i_new]["value"]
                ) {
          ...
          if (error_message && error_message.length == 0) {
            let new_variation = Object.assign(
              {},
              this.variation_item.new_variation
            );
            new_variation.is_selected = false;
            this.item.product_variations.structured_variations.push(
              new_variation
            );
          ...
          let temp = {
            structured_variations: [
              Object.assign({}, this.variation_item.new_variation),
            ],
            all_attribute_name:
              this.variation_item.create_variation_data.all_attribute_name,
          };
    ...
    afterGetAssets(data, res) {
      if (data) {
        ...
        if (
          ...
          this.route.query.filter.quantity
        ) {
    ...
    convertToTreeSelectFormat(data) {
      ...
      data.forEach((category) => {
        ...
        if (category.sub_categories && category.sub_categories.length > 0) {
          categoryItem.children = this.convertToTreeSelectFormat(
    ...
    setParentId() {
      if (this.item.categories) {
        const checkedItem = Object.entries(this.item.categories).find(
    ...
    async getList() {
      ...
      await vaah().ajax(this.ajax_url, this.afterGetList, options);
    },
    ...
    async getItem(id) {
      if (id) {
        await vaah().ajax(ajax_url + "/" + id, this.getItemAfter);
      }
    },
    //---------------------------------------------------------------------
    async getItemAfter(data, res) {
      if (data) {
        this.item = data;
        if (
          this.categories_dropdown_data &&
          this.categories_dropdown_data.length > 0
        ) {
          this.item.categories = this.convertToTreeSelectData(
    ...
    async updateList(type = null) {
      ...
      await vaah().ajax(this.ajax_url, this.updateListAfter, options);
    },
    //---------------------------------------------------------------------
    ...
    async listAction(type = null) {
      ...
      await vaah().ajax(url, this.updateListAfter, options);
    },
    //---------------------------------------------------------------------
    ...
    async attachVendors(item) {
      ...
      await vaah().ajax(ajax_url, this.attachVendorsAfter, options);
    },
    ...
    async saveVariation(id) {
      ...
      await vaah().ajax(ajax_url, this.saveVariationAfter, options);
    },
    ...
    async removeCategory(type, item = null) {
      ...
      await vaah().ajax(ajax_url, this.removeCategoryAfter, options);
    },
    ...
    async itemAction(type, item = null) {
      ...
      await vaah().ajax(ajax_url, this.itemActionAfter, options);
    },
    //---------------------------------------------------------------------
    ...
    compareList(prev_list, current_list) {
      const removed_Items = prev_list.filter(
        (previous_item) =>
          !current_list.some(
      ...
      const removed_item_present_in_current_list = removed_Items.some(
        (removed_item) =>
          current_list.some(
            (current_item) => current_item.id === removed_item.id
          )
      );
      if (!removed_item_present_in_current_list) {
        this.action.items = this.action.items.filter(
          (item) =>
    ...
    async getFormInputs() {
      ...
      await vaah().ajax(url, this.getFormInputsAfter);
    },
    //---------------------------------------------------------------------
      ...
      if (data) {
        ...
        if (data.fill.category) {
          ...
          this.item.categories =
            this.convertToTreeSelectData(category_after_fill);
        }
    ...
    getCategoriesAfter(data, res) {
      this.categories_dropdown_data = this.convertToTreeSelectFormat(
        data.categories
      );
    },
    //---------------------------------------------------------------------
    ...
    addProductVariation() {
      ...
      const product_variations_slugs = unique_product_variations.map(
    ...
    addProductVendor() {
      ...
      const product_vendors_slugs = unique_vendors.map(
    ...
    watchAttributes() {
      watch(
        this.variation_item,
        (newVal, oldVal) => {
          const anyDeselected = newVal.product_attributes.some(
    ...
    watchVariations() {
      watch(
        ...
        (newVal, oldVal) => {
          const anyDeselected =
            newVal.product_variations.structured_variations.some(
              (item) => !item.is_selected
            );
    ...
    watchVendors() {
      watch(
        this.item,
        (newVal, oldVal) => {
          const anyDeselected = newVal.vendors.some(
            (item) => !item.is_selected
          );
    ...
    selectCategoryForFilter(event) {
      ...
      const existing_category = selected_categories.find(
        (category) => category === event.data.toLowerCase()
      );
    ...
    removeCategoryForFilter(event) {
      let selected_categories = Array.isArray(this.query.filter.category)
        ? [...this.query.filter.category]
        : [];

      if (Array.isArray(selected_categories)) {
        const index = selected_categories.findIndex(
          (category) => category === event.data.toLowerCase()
        );
    ...
    setCategoryInFilterAfterRefreshAfter(data, res) {
      if (data) {
        this.product_category_filter =
          this.convertFilterCategoryToTreeSelect(data);
      }
    },
    //---------------------------------------------------------------------
    ...
    async vendorPreferredAction(action, vendor_item = null) {
      ...
      ajax_url +=
        "/" +
        vendor_item.vh_st_product_id +
        "/vendors/" +
        vendor_item.id +
        "/action";
      await vaah().ajax(ajax_url, this.vendorPreferredActionAfter, options);
    },
    //---------------------------------------------------------------------
    ...
    async topSellingProducts(selected_store_id = null) {
      let options = {
        query: {
          ...
          selected_store:
            selected_store_id ?? this.query?.selected_store ?? null,
        },
      };
      await vaah().ajax(
    ...
    async exportProducts(type, data) {
      ...
      await vaah().ajax(url, this.exportProductsAfter, options);
    },
