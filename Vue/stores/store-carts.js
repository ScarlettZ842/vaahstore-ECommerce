...
let ajax_url = base_url + "/api/store/carts";
...
export const useCartStore = defineStore({
  ...
  getters: {
    ...
    displayedAddresses() {
      const sorted_addresses = this.shipping_addresses.sort(
        (a, b) => b.is_default - a.is_default
      );
      const default_address = sorted_addresses.find(
    ...
    displayedBillingAddresses() {
      const sorted_addresses = this.user_saved_billing_addresses.sort(
        (a, b) => b.is_default - a.is_default
      );
      const default_address = sorted_addresses.find(
        (address) => address.is_default === 1
      ...
      return this.show_all_billing_address
        ? sorted_addresses
        : sorted_addresses.slice(0, 2);
    },
    ...
    accordionHeader() {
      if (this.is_editing) {
        if (
          this.editing_address &&
          this.user_saved_billing_addresses.includes(this.editing_address)
        ) {
        ...
        if (this.shipping_addresses.length === 0) {
          ...
          this.show_tab_for_billing &&
          this.user_saved_billing_addresses.length >= 0
        ) {
  ...
  actions: {
    ...
    afterGetAssets(data, res) {
      if (data) {
        ...
        // API failed (e.g., 404 before VaahCMS installation)
        // Set minimal default assets to allow page rendering
        this.assets = {
          rows: 10,
          empty_item: {},
          countries: [],
    ...
    async getItem(id, selected_store = null) {
      ...
      options.query.selected_store =
        selected_store || this.query?.selected_store;
      if (id) {
        await vaah().ajax(ajax_url + "/" + id, this.getItemAfter, options);
      }
    },
    //---------------------------------------------------------------------
    ...
    async updateQuantity(pivot_data, event) {
      ...
      const query = {
        products: (Array.isArray(pivot_data) ? pivot_data : [pivot_data]).map(
          (item) => ({
            ...item,
    ...
    async getCartItemDetailsAtCheckoutAfter(data, res) {
      if (data) {
        ...
        this.item_new_billing_address = vaah().clone(
          this.assets.empty_item.item_billing_address
        );
        if (data.user_addresses) {
          this.shipping_addresses = data.user_addresses;
          const defaultAddress = data.user_addresses.find(
            (address) => address.is_default === 1
          );
          this.user_address =
            defaultAddress ||
            data.user_addresses[
              Math.floor(Math.random() * data.user_addresses.length)
            ];
        }
        ...
        if (data && data.user_billing_addresses) {
          ...
          const defaultBillingAddress = data.user_billing_addresses.find(
            (address) => address.is_default === 1
          );
          this.user_billing_address =
            defaultBillingAddress ||
            data.user_billing_addresses[
              Math.floor(Math.random() * data.user_billing_addresses.length)
            ];
        }
        this.$router.push({
    ...
    handleSameAsShippingChange() {
      if (
        this.selected_shipping_address !== undefined &&
        this.is_same_as_shipping !== undefined
      ) {
        if (this.is_same_as_shipping) {
          this.item_billing_address = { ...this.selected_shipping_address };
        } else if (
          Array.isArray(this.is_same_as_shipping) &&
          this.is_same_as_shipping.length === 0
        ) {
          this.item_new_billing_address = vaah().clone(
            this.assets.empty_item.item_billing_address
          );
    ...
    async addToWishList(item, user) {
      const is_wishlisted =
        item.is_wishlisted === 1 || item.is_wishlisted === true;
      const query = {
        item_detail: item,
        user_detail: user,
