...
let ajax_url = base_url + "/api/store/stores";
...
export const useStoreStore = defineStore({
  ...
  actions: {
    ...
    setViewAndWidth(route_name) {
      this.view = "list";
      if (
        route_name.includes("stores.view") ||
      ...
      if (this.item) {
        ...
        watch(
          ...
          (newVal, oldVal) => {
            let flag = 1;
            if (
              this.item.currency_default &&
              this.item.currency_default.length > 1
            ) {
        ...
        watch(
          ...
          (newVal, oldVal) => {
            let flag = 1;
            if (
              this.item.language_default &&
              this.item.language_default.length > 1
            ) {
    ...
    searchCurrencies(event) {
      ...
      this.currency_suggestion_list = this.currencies_list.filter((item) => {
        return (
          ...
          (!this.item.default_currency ||
            item.name !== this.item.default_currency.name)
        );
      });
    },
    ...
    async saveCurrencies() {
      ...
      if (unique_currencies.length == 0) {
        vaah().toastErrors([
          "Currency is required when Is Multi Currency is true",
        ]);
        await this.getItem(this.route.params.id);
        return false;
      }
    ...
    async saveLanguages() {
      ...
      if (unique_languages.length === 0) {
        vaah().toastErrors([
          "Language is required when Is Multi Language is true",
        ]);
        await this.getItem(this.route.params.id);
        return false;
      }
    ...
    searchLanguages(event) {
      ...
      this.language_suggestion_list = this.languages_list.filter((item) => {
        return (
          item.name.toLowerCase().includes(query) &&
          (!this.item.default_language ||
            item.name !== this.item.default_language.name)
        );
      });
    },
    ...
    searchCurrencyDefault(event) {
      setTimeout(() => {
        if (!event.query.trim().length) {
          ...
          this.currency_default_suggestion_list = this.item.currencies.filter(
            (department) => {
              return department.name
                .toLowerCase()
                .startsWith(event.query.toLowerCase());
    ...
    searchCurrency(event) {
      setTimeout(() => {
        if (!event.query.trim().length) {
          ...
          this.currency_suggestion_list = this.currencies.filter(
            (department) => {
              return department.name
                .toLowerCase()
                .startsWith(event.query.toLowerCase());
    ...
    compareList(prev_list, current_list) {
      ...
      const removed_items = prev_list.filter(
        (item) => !current_set.has(item.id)
      );

      this.action.items = this.action.items.filter((item) =>
        current_set.has(item.id)
      );
