...
let ajax_url = base_url + "/api/store/orders";
...
export const useOrderStore = defineStore({
  ...
  actions: {
    ...
    setViewAndWidth(route_name) {
      ...
      if (
        route_name.includes("orders.view") ||
    ...
    watchRoutes(route) {
      ...
      this.watch_stopper = watch(
        ...
        (newVal, oldVal) => {
          if (
            this.watch_stopper &&
            !newVal.name.startsWith(this.route_prefix)
          ) {
            this.watch_stopper();
    ...
    fetchSalesChartDataAfter(data, res) {
      ...
      const series_data = data.chart_series.orders_sales_chart_data.map(
        (series) => ({
          name: series.name,
    ...
    fetchOrderPaymentsDataAfter(data, res) {
      if (
        !data ||
        !Array.isArray(
          data.order_payments_chart_series?.orders_payment_income_chart_data
        )
      ) {
        return;
      }
      ...
      this.income_growth_rate =
        data.order_payments_chart_series?.income_growth_rate;

      const series_data =
        data.order_payments_chart_series.orders_payment_income_chart_data.map(
          (series) => ({
            name: series.name,
      ...
      this.updateOrderPaymentsIncomeChartOptions(
        updated_order_payments_income_chart_options
      );
    },
    ...
    fetchOrdersCountChartDataAfter(data, res) {
      ...
      const series_data = [
        {
          ...
          type: "line",
          data:
            data.chart_series.orders_count_bar_chart.find(
              (s) => s.name === "Created"
            )?.data || [],
        },
        {
      ...
      const totals = {
        total_orders:
          series_data
            .find((series) => series.name === "Created")
            ?.data.reduce((sum, item) => sum + item.y, 0) || 0,
        completed_orders:
          series_data
            .find((series) => series.name === "Completed")
            ?.data.reduce((sum, item) => sum + item.y, 0) || 0,
      };
    ...
    async downloadInvoice(order) {
      try {
        ...
        if (error.response && error.response.status === 404) {
          ...
          vaah().toastErrors([
            "Failed to download invoice. Please check if the order exists or try again.",
          ]);
        }
      }
    },
